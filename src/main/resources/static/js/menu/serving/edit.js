$(function() {
    ...
    $("#ingredient-modal").on("click", "a[href*='edit']", function(e) {
        ...
        modal.find(".ingredient option").filter(function() {
        ...
        form.find("button.edit").click(function() {
            ...
            URLUtil.put(SERVING_URL+id, model).then(function(obj) {
                tr.find(".amount").html(obj.numberOfUnits);
                tr.find(".name").html(obj.ingredient.name);

                form.find(".amount").val("");
                form.find(".ingredient").val("1");
